...
function buildShareText(result: RoastResult): string {
  const { score, llm, url } = result;
  const hostname = (() => {
...
export function ShareBar({ result }: Props) {
  ...
  const toolUrl = typeof window !== "undefined" ? window.location.origin : "https://landing-roaster.vercel.app";
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-6 space-y-4">
      ...
      <div className="bg-gray-50 border border-gray-100 rounded-xl p-4">
        <p className="text-xs text-gray-500 whitespace-pre-line leading-relaxed">{shareText}</p>
      </div>
